import { DocAndGitLinks } from '../DocAndGitLinks'

export const Footer = () => {
  return (
    <div className='bg-primary-light dark:bg-black text-white'>
      <div className='grid grid-flow-row align-middle mx-auto w-5/6 2xl:w-3/6'>
        <div className='grid grid-flow-col gap-4 justify-between items-center py-4'>
          <div className='flex gap-4 text-[12px]'>
            <DocAndGitLinks />
          </div>

          <p className='text-[12px]'>© 2022 SUSE. Open Zero Trust is an open source project of the SUSE Rancher Engineering group.</p>
          {/* <SuseLogoSVG className='w-40 h-20 fill-current text-white' /> */}
        </div>
      </div>
    </div>
  )
}
